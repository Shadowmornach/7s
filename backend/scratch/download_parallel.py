...
print(f"Total file size: {total_size / (1024*1024):.2f} MB. Spawning {num_chunks} parallel download threads...")
...
def download_range(arg):
    ...
    with urllib.request.urlopen(req, timeout=60) as resp, open(temp_file, 'wb') as f:
        f.write(resp.read())
...
print(f"PARALLEL DOWNLOAD COMPLETE! File size: {size_mb:.2f} MB", flush=True)
